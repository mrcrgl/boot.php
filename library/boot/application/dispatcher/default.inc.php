...
class RequestDispatcher 
{
    ...
    public function __construct()
   {
    ...
    protected function prepareEnv()
         ...
         * prepare Smarty
         */
        #BInstance::_new(new Smarty(), 'template');
        /**/
    ...
    protected final function checkBrowser()
    {
        ...
        if ($browser_info['browser'] == 'MSIE' && ($browser_info['version'] < 8) ) {
            BFactory::getTemplate()->display('incompatible_user_agent.htpl');
            exit;
        }
    ...
    {
        #BFactory::getTemplate()->assign('default_landing_page');
        if ((Validator::is($this->arrView[$_view][1], 'array') || $this->arrView[$_view][1] == true) && !BInstance::f('Login')->loggedIn()) {
            if (!in_array(strtolower($this->_SERVER['REQUEST_URI']), array('/', '/index.html'))) {
                BFactory::getTemplate()->assign('requested_view', $this->_SERVER['REQUEST_URI']);
                BFactory::getTemplate()->assign('error', "loginRequired");
            } else {
                BFactory::getTemplate()->assign('requested_view', BInstance::f('LinkGenerator')->getLink($this->strLandingPage));
            }
    ...
    protected function checkPermission($_view)
    {
        ...
        foreach ($this->arrView[$_view][1] as $right) {
            ...
            if (BInstance::f('Login')->obj->hasPermission($right)) {
                $hasPermission = true;
            }
        }

        if ($hasPermission === false) {
            BInstance::f('Login')->obj->log($this, "Permission denied by changing View to ".$this->arrView[$_view][0]);

            BFactory::getTemplate()->assign('requested_view', $this->_SERVER['REQUEST_URI']);
            BFactory::getTemplate()->assign('system_error', "permissionDenied");
    ...
    {
        if ( BSettings::f('default.debug', false) ) {
            /* TODO
             *
             * BFactory::getTemplate()->assign('debug', ENV::$debug);
            BFactory::getTemplate()->assign('count_read_querys', ENV::$countSQLQuerys['r']);
            BFactory::getTemplate()->assign('count_write_querys', ENV::$countSQLQuerys['w']);
            BFactory::getTemplate()->assign('count_querys', ENV::$countSQLQuerys['r'] + ENV::$countSQLQuerys['w']);
            BFactory::getTemplate()->assign('SQLQueryLog', ENV::$SQLQueryLog);
            BFactory::getTemplate()->assign('generate_time', (microtime() - ENV::$startTime));*/
        }
    }
