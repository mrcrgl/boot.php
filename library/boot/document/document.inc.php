...
class BDocument 
{
    ...
    public function __construct()
   {
    ...
    public static function getInstance($type='html')
    {
        ...
        $classname = 'BDocument'.ucfirst($type);
        ...
        if (!class_exists($classname)) 
{
    ...
    public function getRenderer($type='smarty')
    {
        ...
        if (!$this->_renderer) {
            ...
            $classname = 'BDocumentRenderer'.ucfirst($this->getType()).ucfirst($type);

            $path = dirname(__FILE__).DS.$this->getType().DS.'renderer'.DS.$type.'.inc.php';

            BLoader::register($classname, $path);
            BLoader::autoload($classname);
...
/**
    ...
    public function getTitle()
     {
        return (($this->title) ? sprintf("%s - %s", $this->title, BSettings::f('default.title', 'Versions 2.0 project')) : BSettings::f('default.title', 'Versions 2.0 project') );
    }
