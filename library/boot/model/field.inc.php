...
#BLoader::discover(dirname(__FILE__).DS.'field');

class BModelField extends BObject 
{
...
{
        BLoader::register($classname, dirname(__FILE__).DS.'field'.DS.(BString::camelcase_to_underscores($type)).'.inc.php');
              ...
              if (!class_exists($classname)) 
{
    ...
    #print "fooo";die("prepareModel called");
      ...
      // INFO: This produces an error. defaults were not set and declaration was saved to db
        ...
        print "</pre>";
        */
        #$ref =& BModelField::getInstance($model_name, 'uid', 'PrimaryKey', array('db_column' => 'uid'));
        ...
        foreach ($model->getFields() as $column) {
            ...
            if (!isset(self::$_instances[$model_name][$column])) {
                if (!preg_match('/^(?P<type>\w+):(?P<options>.*)$/', $declaration, $matches)) {
                    printf("BModel column declataion layout mismatch: %s<br />", $declaration);
                    print "Column: $column".NL;
                    var_dump($declaration);print "<br />";
                    #throw new Exception(sprintf("BModel column declataion layout mismatch: %s", $declaration));
                }
                #print "<br /><br />";
                #var_dump($matches);print "<br />";


                $options = BArray::parseOptions(&$matches['options']);
                ...
                $ref =& BModelField::getInstance($model_name, $column, $matches['type'], $options);
            } else {
                $ref =& BModelField::getInstance($model_name, $column);
            }
