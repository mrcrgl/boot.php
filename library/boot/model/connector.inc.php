 ...
 * --
 */
abstract class BModelConnector extends BModelDefault 
{
                ...
                if (!class_exists($classname)) 
{
     ...
     * User Log
     */
    if (get_class($this) != 'UserLog' && is_object(BInstance::f('Login')) && is_object(Instance::f('Login')->obj)) 
{
      BInstance::f('Login')->obj->log($this, (($bOnCreate === true) ? "Object created." : "Object updated."));
    }
  ...
  public function isSqlInstalled()
  {
      ...
      $this->objDB =& BFactory::getDatabase();
  ...
  {
      return BFactory::getLanguage();
      /*if (BInstance::f('DataLanguage') && BInstance::f('DataLanguage')->isValid()) {
          return BInstance::f('DataLanguage');
      }
      if (BInstance::f('Language') && BInstance::f('Language')->isValid()) {
          return BInstance::f('Language');
      }
  ...
  protected function updateRelationKeyValue($param, $dbLayout)
   {
    if ( !is_object($this->objDB) ) {
      throw new Exceptoin("\$this->objDB is not a Valid 'BDatabase' instance.");
    }
  ...
  {
    if ( !is_object($this->objDB) ) {
      throw new Exception("\$this->objDB is not a Valid 'BDatabase' instance.");
    }
  ...
  protected function openDatabase()
  {
    ...
    $this->objDB = BFactory::getDatabase($this->_DataMap['_database']);
