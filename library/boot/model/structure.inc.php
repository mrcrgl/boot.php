...
class BModelStructure extends BObject 
{
    ...
    public function __get($__var)
    {
        if ($__var == 'objects') {
            if (is_null($this->_manager))
                $this->_manager = BModelManager::getInstance(&$this);
        ...
        if (substr($__var, -5, 5) == '__set') {
          ...
          if (!$this->_allow_cache || !isset($this->_related_manager[$__var])) {
            ...
            if ($reference) {
              $related = new $reference();// BModelStorage::_($reference);
            } else {
              $related_name = BString::underscores_to_camelcase(substr($__var, 0, -5));
              $related = new $related_name(); //BModelStorage::_($related_name);
            }
            $this->_related_manager[$__var] = BModelManager::getInstance(&$this, $related, 'related');
          }
...
{
        $this->_field_declarations[$__field] =& BModelField::getInstance($this->getClass(), $__field);
      }
