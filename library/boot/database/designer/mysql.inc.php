...
class BDatabaseDesignerMysql extends BDatabaseDesigner 
{
      ...
      if (!in_array($this->getTableName(get_class($model)), $tables_installed)) 
{
    ...
    public function getCreateTable(&$model)
  {
    ...
    {
        if (!is_object($model) || !is_subclass_of(get_class($model), 'BModelStructure')) 
{
            throw new Exception( sprintf("Model %s (Type: %s) is not a valid Object or parent of BModelStructure", get_class($model), get_type($model)) );
        }
    ...
    public function getModels($model, $filter=array())
     {
        ...
        while ($dbo->nextRecord()) {
            $return[$i] = new $model_name(); //BModelStorage::_($model_name);
    ...
    public function getTableName($model_name)
    {
        ...
        $parts = BString::explode_camelcase($model_name);
        ...
        return BString::strtolower(implode('_', $parts));
    }
}