...
class BDatabaseMysql extends BDatabase 
{
  ...
  function __construct($host=null, $database=null, $user=null, $pass=null)
   {
    ...
    // put the data into the related class vars
      ...
      if (is_null($host) || is_null($database)) {
          $host            = BSettings::f('database.host');
          $database = BSettings::f('database.database');
          $user            = BSettings::f('database.user');
          $pass            = BSettings::f('database.password');
      }
  ...
  function sql_abort($strMessage)
   {
    ...
    BResponse::error(500, 'Database error.', $message);
  ...
  protected function query($strQueryString)
   {
    if (!$this->displayOnly) {
      ...
      if (BSettings::f('database.debug')) {
        $profiler = BProfiler::getInstance('db');
      }
