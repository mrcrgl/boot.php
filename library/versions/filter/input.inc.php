...
/**
 * VFilterInput is a class for filtering input from any data source
 *
 * @package     Versions.core
 * @subpackage  Filter
 * @since       2.0
 */
class VFilterInput extends VObject {
	/**
	 * @var    array  A container for VFilterInput instances.
	 * @since  11.3
	 */
   ...
   * @param   integer  $xssAuto     Only auto clean essentials = 0, Allow clean blacklisted tags/attr = 1
   *
   * @return  VFilterInput  The VFilterInput object.
   *
   * @since   2.0
   */
  public static function &getInstance($tagsArray = array(), $attrArray = array(), $tagsMethod = 0, $attrMethod = 0, $xssAuto = 1) {
  	...
  	if (empty(self::$instances[$sig])) {
  		self::$instances[$sig] = new VFilterInput($tagsArray, $attrArray, $tagsMethod, $attrMethod, $xssAuto);
  	}
