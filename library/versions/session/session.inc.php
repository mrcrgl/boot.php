...
#VLoader::register('VSessionStorage', dirname(__FILE__) . '/storage.inc.php');

VLoader::import('versions.environment.request');
VLoader::import('versions.session.storage');
...
class VSession extends VObject
{
    ...
    /**
     * @var    VSession  VSession instances container.
     * @since  11.3
     */
    ...
    public static function getStores()
     {
        VLoader::import('joomla.filesystem.folder');
    ...
    public function destroy()
     {
        ...
        if (isset($_COOKIE[session_name()])) {
            $cookie_domain = VSettings::f('default.cookie_domain', '');
            $cookie_path = VSettings::f('default.cookie_path', '/');
    ...
    protected function _setCookieParams()
     {
        ...
        if (VSettings::f('cookie_domain', '') != '') {
            $cookie['domain'] = VSettings::f('cookie_domain');
        }

        if (VSettings::f('cookie_path', '') != '') {
            $cookie['path'] = VSettings::f('cookie_path');
        }
        session_set_cookie_params($cookie['lifetime'], $cookie['path'], $cookie['domain'], $cookie['secure']);
    }
