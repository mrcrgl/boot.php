...
class VSettingsIni implements VSettingsInterface {
	...
	static function import_path($__path, $__file='settings') {
		...
		if (!is_dir($__path)) {
			...
			#VDebug::_(new VDebugMessage(sprintf("path not found: '%s'", $__path)));
		}
		...
		self::$collection = array_merge(self::$collection, parse_ini_file($settings_file, true));
	}
